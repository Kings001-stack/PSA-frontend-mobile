...
const AdvertManagementScreen: React.FC = () => {
  ...
  const handleEdit = (advert: Advert) => {
    ...
    setSelectedImage(
      advert.image_url ||
        (advert.image_path ? `${STORAGE_URL}/${advert.image_path}` : null),
    );
    setFormData({
      title: advert.title,
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              setErrorMessage("Failed to delete advert");
              setShowError(true);
            }
          },
        },
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSuccessMessage(
        `Advert ${isEditing ? "updated" : "created"} successfully`,
      );
      setShowSuccess(true);
      setTimeout(() => {
        fetchAdverts();
  ...
  const getStatusColor = (advert: Advert) => {
    if (!advert.is_active)
      return { bg: "#fee2e2", text: "#dc2626", label: "INACTIVE" };
    ...
    if (now < start)
      return { bg: "#fef3c7", text: "#d97706", label: "SCHEDULED" };
    ...
    return (
      <View style={styles.advertCard}>
        {(item.image_url || item.image_path) && (
          <Image
            source={{
              uri: item.image_url || `${STORAGE_URL}/${item.image_path}`,
            }}
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtext}>
              Create your first promotional advert
            </Text>
          </View>
        }
      />
