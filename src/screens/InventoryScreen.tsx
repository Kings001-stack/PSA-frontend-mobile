...
const InventoryScreen: React.FC = () => {
  ...
  const handleAdd = () => {
    ...
    setFormData({
      ...
      quantity: "",
      batch_number:
        "B-" + Math.random().toString(36).substring(2, 7).toUpperCase(),
  ...
  const handleEdit = (item: InventoryItem) => {
    ...
    setFormData({
      ...
      unit_price: (item as any).unit_price?.toString() || "",
    });
    setIsModalVisible(true);
  };
  ...
  const handleSubmit = async () => {
    if (
      (!formData.medication_id && !formData.medication_name.trim()) ||
      !formData.quantity ||
      !formData.batch_number
    ) {
      Alert.alert(
        "Missing Fields",
        "Please provide a medication name and quantity.",
      );
      return;
    }
    ...
    try {
      if (isEditing && selectedItem) {
        ...
        if (formData.quantity) {
          updateData.quantity = parseInt(formData.quantity, 10);
        }
        if (formData.reorder_level) {
          updateData.reorder_level = parseInt(formData.reorder_level, 10);
        }
        if (formData.batch_number) {
          updateData.batch_number = formData.batch_number;
        }
        if (formData.expiry_date) {
          updateData.expiry_date = formData.expiry_date;
        }
        if (formData.unit_price) {
          updateData.unit_price = parseFloat(formData.unit_price);
        }

        console.log("Updating inventory with data:", updateData);
        await api.patch(`/admin/inventory/${selectedItem.id}`, updateData);
      ...
      setErrorMessage(error.response?.data?.message || "Failed to save data");
      setShowError(true);
    }
  };
            ...
            <ScrollView style={styles.formContent} bounces={false}>
              ...
              {!isEditing ? (
                <View>
                  ...
                  {formData.medication_name.length > 0 &&
                    medications.filter((m) =>
                      m.name
                        .toLowerCase()
                        .includes(formData.medication_name.toLowerCase()),
                    ).length > 0 && (
            ...
            <View style={styles.modalFooter}>
              ...
              <TouchableOpacity style={styles.submitBtn} onPress={handleSubmit}>
                <Text style={styles.submitBtnText}>
                  {isEditing ? "Update Stock" : "Add to Inventory"}
                </Text>
              </TouchableOpacity>
            </View>
          </KeyboardAvoidingView>
